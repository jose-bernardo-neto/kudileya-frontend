...
const getApiBaseUrl = (): string => {
	...
	if (import.meta.env.PROD || import.meta.env.VITE_USE_PROD_API === 'true') {
		return (
			...
			'https://kudileya-app-backend.onrender.com'
		);
	}
...
export const config: AppConfig = {
	...
	contact: {
		...
		whatsappMessage:
			...
			'Olá! Gostaria de saber mais sobre os serviços da Kudileya.',
		supportEmail: import.meta.env.VITE_SUPPORT_EMAIL || 'contato@kudileya.com',
		companyWebsite:
			import.meta.env.VITE_COMPANY_WEBSITE || 'https://kudileya.com',
	},
	...
	features: {
		...
		enableAnimations: parseBoolean(
			import.meta.env.VITE_ENABLE_ANIMATIONS,
			true,
		),
	},
	...
	ui: {
		defaultLanguage:
			(import.meta.env.VITE_DEFAULT_LANGUAGE as 'pt' | 'en') || 'pt',
		defaultTheme:
			(import.meta.env.VITE_DEFAULT_THEME as 'light' | 'dark') || 'light',
	...
	documents: {
		...
		allowedFileTypes:
			import.meta.env.VITE_ALLOWED_FILE_TYPES || 'application/pdf',
	...
	security: {
		...
		forceHttps: parseBoolean(
			import.meta.env.VITE_FORCE_HTTPS,
	...
	// Development
	dev: {
		showErrorDetails: parseBoolean(
			import.meta.env.VITE_SHOW_ERROR_DETAILS,
			import.meta.env.DEV,
		),
