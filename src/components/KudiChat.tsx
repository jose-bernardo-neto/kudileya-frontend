...
const KudiChat = ({ initialQuestion }: KudiChatProps) => {
	...
	const sendMessage = async () => {
		...
		try {
			...
			const aiResponse =
				data.answer || 'Desculpe, não consegui processar sua pergunta.';
			...
			const fallbackMessage: Message = {
				id: (Date.now() + 1).toString(),
				content: t('kudichat.fallback').replace(
					'{question}',
					userMessage.content,
				),
				sender: 'ai',
				...
				<div className='p-4 bg-card border-t border-border'>
					<div className='flex space-x-2'>
						<Input
							...
							placeholder={
								isListening
									? t('kudichat.listening')
									: t('kudichat.placeholder')
							}
						...
						{isSpeechSupported && (
							<Button
								...
								size='icon'
								title={
									isListening ? t('kudichat.listening') : t('kudichat.listen')
								}
