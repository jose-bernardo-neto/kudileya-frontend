...
const DocumentsUseful = () => {
	...
	const {
		...
		queryFn: async (): Promise<DocumentsResponse> => {
			...
			const response = await fetch(apiHelpers.getApiUrl('/api/v1/documents'));
	...
	const categories = [
		...
		{
			id: 'leis',
			label: t('documents.category.laws'),
			icon: '📜',
			color: 'bg-blue-100',
		},
		{
			id: 'formularios',
			label: t('documents.category.forms'),
			icon: '📋',
			color: 'bg-green-100',
		},
		{
			id: 'manuais',
			label: t('documents.category.manuals'),
			icon: '📖',
			color: 'bg-purple-100',
		},
		{
			id: 'contratos',
			label: t('documents.category.contracts'),
			icon: '📝',
			color: 'bg-orange-100',
		},
		{
			id: 'processos',
			label: t('documents.category.processes'),
			icon: '⚖️',
			color: 'bg-red-100',
		},
		{
			id: 'outros',
			label: t('documents.category.others'),
			icon: '📄',
			color: 'bg-yellow-100',
		},
	];
	...
	const getCategoryInfo = (categoryId?: string) => {
		return (
			...
			categories.find((cat) => cat.id === 'outros')!
		);
	};
	...
	const generateThumbnail = (doc: Document) => {
		...
		return (
			<div
				className={`w-full h-24 ${categoryInfo.color} rounded-lg flex items-center justify-center border-2 border-dashed border-gray-300`}
			>
					...
					<>
										...
										>
											<CardContent className='p-4'>
												...
												<div className='flex items-start justify-between mb-3'>
													...
													{document.isUseful && (
														<Star
															className='pl-2 text-yellow-500 fill-yellow-500'
															size={20}
														/>
													)}
												</div>
												...
												<div className='flex items-center gap-2 mb-3'>
													<Badge
														variant='secondary'
														className={`text-xs ${categoryInfo.color} border-none`}
													>
						...
						{filteredDocuments.length > 0 && (
							<div className='mt-6 text-center text-sm text-muted-foreground'>
								{t('documents.results.showing')} {filteredDocuments.length}{' '}
								{t('documents.results.of')} {documents.length}{' '}
								{t('documents.results.documents')}
							</div>
						)}
					</>
				)}
